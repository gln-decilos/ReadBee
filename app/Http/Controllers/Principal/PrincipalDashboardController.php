...
class PrincipalDashboardController extends Controller
{
        ...
        if (! $schoolId) {
            if ($request->expectsJson() || $request->query('ajax')) {
                return response()->json([
                    'success' => false,
                    'message' => 'Your principal account is not connected to a school. Please sign in again or contact the administrator.',
                ], 401);
            }

            return redirect()->route('signin')
                ->with('error', 'Please sign in as a principal to view the dashboard.');
        }

        $dashboardData = $this->buildDashboardData($schoolId, $request);
    ...
    private function buildDashboardData(string $schoolId, Request $request): array
    {
        $allAssignments = $this->fetchSchoolAssignments($schoolId);
    ...
    /**
     * Build the principal dashboard scope from the school's own sections and quarters.
     *
     * Important: this intentionally does not depend on assigned_evaluators. A principal
     ...
     */
    private function fetchSchoolAssignments(string $schoolId): array
    {
        ...
        if (! $sectionsResponse->successful()) {
            report('Failed to fetch principal dashboard school sections: ' . $sectionsResponse->body());
            return [];
        }

        $sections = collect($sectionsResponse->json())
            ->filter(fn ($section) => ! empty($section['section_id']) && ! empty($section['year_id']))
            ->values();
        ...
        if (! $quartersResponse->successful()) {
            report('Failed to fetch principal dashboard school year quarters: ' . $quartersResponse->body());
            return [];
        }
        ...
        return $sections
            ->flatMap(function ($section) use ($quartersByYear) {
                $quarters = $quartersByYear->get($section['year_id'], collect());

                return $quarters->map(function ($quarter) use ($section) {
                    return [
                        'assignment_id' => null,
                        'schedule_id' => null,
            ...
            ->filter(fn ($scope) => ! empty($scope['section_id']) && ! empty($scope['year_id']) && ! empty($scope['quarter_id']))
            ->values()
            ->all();
    }
    ...
    private function attentionLists($records): array
    {
        $oral = $records
            ...
            ->map(fn ($record) => [
                ...
                'grade' => $record['grade_section'] ?? 'School section',
    ...
    private function fetchAssignedPupils(array $sectionIds): array
    {
        ...
        if (! $response->successful()) {
            report('Failed to fetch principal dashboard pupils: ' . $response->body());
            return [];
        }
    ...
    private function fetchAssessmentRecords(array $pupilIds, array $yearIds, array $quarterIds): array
    {
        ...
        if (! $response->successful()) {
            report('Failed to fetch principal dashboard assessment records: ' . $response->body());
            return [];
        }
    ...
    private function fetchRowsByIds(string $table, string $idField, array $ids, string $select): array
    {
        ...
        if (! $response->successful()) {
            report("Failed to fetch {$table} for principal dashboard: " . $response->body());
            return [];
        }
