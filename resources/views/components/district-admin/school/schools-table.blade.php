...
<div x-data='{
    ...
    handleSelectAll() {
        ...
        this.selectedRows = this.selectAll
            ? this.schools.map(s => String(s.school_id))
            : [];
    },
    ...
    handleRowSelect(id) {
        ...
        this.selectAll = this.selectedRows.length === this.schools.length && this.schools.length > 0;
    },
    ...
    filterMunicipalitiesByDistrict(districtId, mode = "create") {
        ...
        if (mode === "create") {
            this.$refs.createForm.querySelector("[name=municipality_id]").value = "";
        } else {
