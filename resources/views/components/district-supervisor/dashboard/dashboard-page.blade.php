@props([
    'dashboardData' => [],
    'dashboardUrl' => route('district-supervisor.dashboard'),
])

<style>
    [x-cloak] { display: none !important; }

    .readbee-district-supervisor-dashboard {
    ...
    .readbee-district-supervisor-dashboard,
    .readbee-district-supervisor-dashboard .readbee-dashboard-hero,
    .readbee-district-supervisor-dashboard .readbee-chart-box,
    .readbee-district-supervisor-dashboard .readbee-stat-card,
    .readbee-district-supervisor-dashboard .readbee-dashboard-mini-card,
    .readbee-district-supervisor-dashboard .readbee-dashboard-pill,
    .readbee-district-supervisor-dashboard select,
    .readbee-district-supervisor-dashboard button,
    ...
    .readbee-district-supervisor-dashboard .apexcharts-canvas,
    .readbee-district-supervisor-dashboard .apexcharts-canvas svg,
    .readbee-district-supervisor-dashboard .apexcharts-canvas svg * {
    ...
    .readbee-district-supervisor-dashboard #supervisorReadingRateChart .apexcharts-series path[stroke],
    .readbee-district-supervisor-dashboard #supervisorReadingRateChart .apexcharts-line-series path,
    .readbee-district-supervisor-dashboard #supervisorReadingRateChart path.apexcharts-line,
    .readbee-district-supervisor-dashboard #supervisorComprehensionRateChart .apexcharts-series path[stroke],
    .readbee-district-supervisor-dashboard #supervisorComprehensionRateChart .apexcharts-line-series path,
    .readbee-district-supervisor-dashboard #supervisorComprehensionRateChart path.apexcharts-line {
        stroke: #1f2937 !important;
        opacity: 1 !important;
    }

    .dark .readbee-district-supervisor-dashboard #supervisorReadingRateChart .apexcharts-series path[stroke],
    .dark .readbee-district-supervisor-dashboard #supervisorReadingRateChart .apexcharts-line-series path,
    .dark .readbee-district-supervisor-dashboard #supervisorReadingRateChart path.apexcharts-line,
    .dark .readbee-district-supervisor-dashboard #supervisorComprehensionRateChart .apexcharts-series path[stroke],
    .dark .readbee-district-supervisor-dashboard #supervisorComprehensionRateChart .apexcharts-line-series path,
    .dark .readbee-district-supervisor-dashboard #supervisorComprehensionRateChart path.apexcharts-line {
...
<script>
    window.districtSupervisorDashboardPage = function(initialState, dashboardUrl) {
        const palette = {
            yellow: '#f2c94c',
            black: '#111827',
            charcoal: '#1f2937',
        ...
        return {
            ...
            availableMunicipalities() {
                ...
                const ids = new Set(this.activeAssignmentLinks()
                    ...
                    .map((assignment) => assignment.municipality_id)
                    .filter(Boolean));

                return this.catalogList('municipalities')
                    .filter((municipality) => ids.has(municipality.municipality_id));
            },
            availableSchools() {
                const selectedYear = this.filters?.school_year_id;
                const selectedQuarter = this.filters?.quarter_id || 'all';
                const selectedMunicipality = this.filters?.municipality_id || 'all';
                const ids = new Set(this.activeAssignmentLinks()
                    .filter((assignment) => !selectedYear || assignment.year_id === selectedYear)
                    .filter((assignment) => selectedQuarter === 'all' || assignment.quarter_id === selectedQuarter)
                    .filter((assignment) => selectedMunicipality === 'all' || assignment.municipality_id === selectedMunicipality)
                    .map((assignment) => assignment.school_id)
                    .filter(Boolean));

                return this.catalogList('schools')
                    .filter((school) => ids.has(school.school_id));
            },
            availableGradeLevels() {
                const selectedYear = this.filters?.school_year_id;
                const selectedQuarter = this.filters?.quarter_id || 'all';
                const selectedMunicipality = this.filters?.municipality_id || 'all';
                const selectedSchool = this.filters?.school_id || 'all';
                const ids = new Set(this.activeAssignmentLinks()
                    .filter((assignment) => !selectedYear || assignment.year_id === selectedYear)
                    .filter((assignment) => selectedQuarter === 'all' || assignment.quarter_id === selectedQuarter)
                    .filter((assignment) => selectedMunicipality === 'all' || assignment.municipality_id === selectedMunicipality)
                    .filter((assignment) => selectedSchool === 'all' || assignment.school_id === selectedSchool)
            ...
            selectedMunicipalityLabel() {
                if ((this.filters?.municipality_id || 'all') === 'all') return 'All Municipalities';
                const municipality = this.findOption(this.availableMunicipalities(), 'municipality_id', this.filters?.municipality_id);
                return municipality?.label || municipality?.municipal_name || 'Municipality';
            },
            selectedSchoolLabel() {
                if ((this.filters?.school_id || 'all') === 'all') return 'All Schools';
                const school = this.findOption(this.availableSchools(), 'school_id', this.filters?.school_id);
                return school?.label || school?.name || 'School';
            },
            ...
            ensureFilterSelections() {
                ...
                const municipalityIds = this.availableMunicipalities().map((municipality) => municipality.municipality_id);
                if ((this.filters.municipality_id || 'all') !== 'all' && !municipalityIds.includes(this.filters.municipality_id)) {
                    this.filters.municipality_id = 'all';
                }

                const schoolIds = this.availableSchools().map((school) => school.school_id);
                if ((this.filters.school_id || 'all') !== 'all' && !schoolIds.includes(this.filters.school_id)) {
                    this.filters.school_id = 'all';
                }
            ...
            renderAllCharts() {
                this.renderBarChart('supervisorReadingLevelChart', 'readingLevel', this.chartData?.readingLevel?.[this.chartFilters.readingLevelSex] || [0, 0, 0], 'Reading Level');
                this.renderBarChart('supervisorComprehensionLevelChart', 'comprehensionLevel', this.chartData?.comprehensionLevel?.[this.chartFilters.comprehensionLevelSex] || [0, 0, 0], 'Comprehension Level');
                this.renderLineChart('supervisorReadingRateChart', 'readingRate', this.chartData?.readingRate?.[this.chartFilters.readingRateSex] || [0, 0, 0, 0], 'Reading Rate');
                this.renderLineChart('supervisorComprehensionRateChart', 'comprehensionRate', this.chartData?.comprehensionRate?.[this.chartFilters.comprehensionRateSex] || [0, 0, 0, 0], 'Comprehension Rate');
                this.renderDonutChart('supervisorMiscueTypeChart', 'miscueType', this.chartData?.miscueDistribution || [0, 0, 0, 0, 0, 0, 0], ['Mispronunciation', 'Omission', 'Substitution', 'Insertion', 'Transposition', 'Reversal', 'Repetition'], 'Miscues');
                this.renderProgressDonut('supervisorFilipinoCompletionChart', 'filipinoCompletion', this.chartData?.filipinoCompletion?.[this.chartFilters.filipinoCompletionSex] || [0, 100], 'Filipino');
                this.renderProgressDonut('supervisorEnglishCompletionChart', 'englishCompletion', this.chartData?.englishCompletion?.[this.chartFilters.englishCompletionSex] || [0, 100], 'English');
                this.renderPieChart('supervisorMaleSpeedChart', 'maleSpeed', this.chartData?.speedMale || [0, 0, 0, 0, 0], ['Fast', 'Average', 'Slow', 'Struggling', 'Non-Reader']);
                this.renderPieChart('supervisorFemaleSpeedChart', 'femaleSpeed', this.chartData?.speedFemale || [0, 0, 0, 0, 0], ['Fast', 'Average', 'Slow', 'Struggling', 'Non-Reader']);
                this.renderPieChart('supervisorComprehensionStatusChart', 'comprehensionStatus', this.chartData?.comprehensionStatus || [0, 0], ['With Comprehension', 'Without Comprehension']);
            },
...
<div
    x-data="districtSupervisorDashboardPage(@js($dashboardData), @js($dashboardUrl))"
    x-cloak
    class="readbee-district-supervisor-dashboard space-y-6"
>
        ...
        <div class="relative z-10 flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
            <div class="min-w-0">
                <div class="flex items-center gap-3">
                    ...
                    <div class="min-w-0">
                        <p class="text-xs font-semibold uppercase tracking-[0.24em] text-gray-500 dark:text-gray-400">District Supervisor Dashboard</p>
                        <h1 class="mt-1 text-2xl font-semibold leading-tight text-gray-950 dark:text-white sm:text-3xl">District Reading Performance Overview</h1>
                    </div>
                </div>
                <p class="mt-4 max-w-3xl text-sm leading-6 text-gray-600 dark:text-gray-300">
                    Monitor reading speed, reading level, comprehension, assessment completion, miscues, and pupils needing support across municipalities and schools under your district.
                </p>
                <div class="mt-4 flex flex-wrap gap-2 text-xs font-medium">
                    ...
                    <span class="readbee-dashboard-pill rounded-full px-3 py-1" x-text="selectedMunicipalityLabel()"></span>
                    <span class="readbee-dashboard-pill rounded-full px-3 py-1" x-text="selectedSchoolLabel()"></span>
                ...
                <template x-teleport="body">
                    ...
                    <div class="mb-4 flex items-start justify-between gap-3">
                        <div>
                            ...
                            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Options are limited to your district scope.</p>
                        </div>
                    ...
                    <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
                        ...
                        <label class="block">
                            <span class="mb-1 block text-xs font-semibold text-gray-600 dark:text-gray-300">Municipality</span>
                            <select x-model="filters.municipality_id" @change="onMunicipalityChange()" class="h-11 w-full rounded-xl border border-gray-300 bg-white px-3 text-sm text-gray-700 shadow-theme-xs focus:border-[#d6b13f] focus:outline-hidden focus:ring-3 focus:ring-amber-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200">
                                <option value="all">All Municipalities</option>
                                <template x-for="municipality in availableMunicipalities()" :key="municipality.municipality_id">
                                    <option :value="municipality.municipality_id" x-text="municipality.label || municipality.municipal_name"></option>
                                </template>
                            </select>
                        </label>

                        <label class="block">
                            <span class="mb-1 block text-xs font-semibold text-gray-600 dark:text-gray-300">School</span>
                            <select x-model="filters.school_id" @change="onSchoolChange()" class="h-11 w-full rounded-xl border border-gray-300 bg-white px-3 text-sm text-gray-700 shadow-theme-xs focus:border-[#d6b13f] focus:outline-hidden focus:ring-3 focus:ring-amber-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200">
                                <option value="all">All Schools</option>
                                <template x-for="school in availableSchools()" :key="school.school_id">
                                    <option :value="school.school_id" x-text="school.label || school.name"></option>
                                </template>
                            </select>
                        </label>
    ...
    <section class="grid grid-cols-1 gap-5 lg:grid-cols-2">
        <div class="readbee-chart-box rounded-2xl border border-gray-200 bg-white p-4 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03] sm:p-5">
            ...
            <div id="supervisorReadingLevelChart" class="readbee-chart-host"></div>
        </div>
        ...
        <div class="readbee-chart-box rounded-2xl border border-gray-200 bg-white p-4 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03] sm:p-5">
            ...
            <div id="supervisorComprehensionLevelChart" class="readbee-chart-host"></div>
        </div>
    </section>
    ...
    <section class="grid grid-cols-1 gap-5 lg:grid-cols-2">
        <div class="readbee-chart-box rounded-2xl border border-gray-200 bg-white p-4 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03] sm:p-5">
            ...
            <div id="supervisorReadingRateChart" class="readbee-chart-host"></div>
        </div>
        ...
        <div class="readbee-chart-box rounded-2xl border border-gray-200 bg-white p-4 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03] sm:p-5">
            ...
            <div id="supervisorComprehensionRateChart" class="readbee-chart-host"></div>
        </div>
    </section>
    ...
    <section class="grid grid-cols-1 gap-5 lg:grid-cols-2">
        <div class="readbee-chart-box rounded-2xl border border-gray-200 bg-white p-4 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03] sm:p-5">
            ...
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Summary of miscues found in district assessment records.</p>
            <div id="supervisorMiscueTypeChart" class="readbee-chart-host mt-2"></div>
        </div>
        <div class="readbee-chart-box rounded-2xl border border-gray-200 bg-white p-4 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03] sm:p-5">
            ...
            <div id="supervisorComprehensionStatusChart" class="readbee-chart-host mt-2"></div>
        </div>
    </section>
    ...
    <section class="readbee-chart-box rounded-2xl border border-gray-200 bg-white p-4 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03] sm:p-5">
        <div class="mb-4 min-w-0">
            ...
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Completion progress for Filipino and English assessments across selected schools.</p>
        </div>
        <div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
            <div class="rounded-2xl border border-gray-200 bg-gray-50 p-4 dark:border-gray-800 dark:bg-white/[0.03]">
                ...
                <div id="supervisorFilipinoCompletionChart" class="readbee-chart-host small-chart"></div>
            </div>
            <div class="rounded-2xl border border-gray-200 bg-gray-50 p-4 dark:border-gray-800 dark:bg-white/[0.03]">
                <div class="mb-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div id="supervisorEnglishCompletionChart" class="readbee-chart-host small-chart"></div>
            </div>
        </div>
    </section>
    ...
    <section class="grid grid-cols-1 gap-5 lg:grid-cols-2">
        <div class="readbee-chart-box rounded-2xl border border-gray-200 bg-white p-4 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03] sm:p-5">
            ...
            <div class="readbee-speed-layout mt-3">
                <div id="supervisorMaleSpeedChart" class="readbee-chart-host small-chart"></div>
        ...
        <div class="readbee-chart-box rounded-2xl border border-gray-200 bg-white p-4 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03] sm:p-5">
            ...
            <div class="readbee-speed-layout mt-3">
                <div id="supervisorFemaleSpeedChart" class="readbee-chart-host small-chart"></div>
