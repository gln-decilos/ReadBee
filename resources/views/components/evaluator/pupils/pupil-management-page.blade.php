...
<div
    x-data="evaluatorPupilManagementPage(@js($schoolYears), @js($selectedYearId), @js($grades), {{ (int) $page }}, {{ (int) $perPage }})"
    x-init="init()"
    x-cloak
        ...
        <div @click.outside="closeEdit" class="w-full max-w-3xl rounded-2xl bg-white p-6 shadow-theme-xl dark:bg-gray-900">
            <div class="flex items-start justify-between gap-4">
                <div>
                    ...
                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Update pupil details. Section changes are limited to your assigned sections in this school year.</p>
                </div>
        ...
        <div class="flex flex-col gap-5 px-6 py-6 lg:flex-row lg:items-end lg:justify-between">
            <div>
                <div class="inline-flex items-center rounded-full bg-yellow-100 px-3 py-1 text-xs font-medium text-yellow-700 dark:bg-yellow-500/10 dark:text-yellow-400">
                    Assigned Section Pupil Management
                </div>
                <h3 class="mt-3 text-xl font-semibold text-gray-800 dark:text-white/90">Pupil Management</h3>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                    Manage only the pupils in your assigned grade and section. Changes are limited to your confirmed evaluator assignments.
                </p>
            </div>
        ...
        <template x-if="viewingGradeId && selectedGrade">
            <div class="px-6 pb-6">
                <div class="mb-6 flex flex-col gap-4 rounded-3xl border border-gray-200 bg-gradient-to-r from-white to-gray-50 p-5 shadow-sm dark:border-white/[0.05] dark:from-gray-900 dark:to-black lg:flex-row lg:items-center lg:justify-between">
                    <div>
                        <div class="inline-flex items-center rounded-full bg-yellow-100 px-3 py-1 text-xs font-medium text-yellow-700 dark:bg-yellow-500/10 dark:text-yellow-400">Assigned Grade Detail</div>
                    ...
                    <button type="button" @click="backToGrades" class="inline-flex items-center justify-center rounded-xl border border-gray-300 px-4 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-white dark:hover:bg-gray-800">Back to Assigned Grades</button>
                </div>
                    ...
                    <aside class="overflow-hidden rounded-3xl border border-gray-200 bg-white dark:border-white/[0.05] dark:bg-white/[0.03]">
                        <div class="flex items-center justify-between gap-3 border-b border-gray-200 px-4 py-4 dark:border-white/[0.05]">
                            <div x-show="!sectionsCollapsed" x-transition.opacity>
                                ...
                                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Your assigned sections</p>
                            </div>
                            ...
                            <div>
                                ...
                                <div x-show="importMode" x-transition class="border-b border-gray-200 p-5 dark:border-white/[0.05]">
                                    <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                                        ...
                                        <a href="{{ route('evaluator.pupils.import.template') }}" class="inline-flex items-center justify-center rounded-xl border border-gray-300 px-4 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-white/5">
                                            Download Template
                                        </a>
                                    </div>
...
<script>
    function evaluatorPupilManagementPage(schoolYears, selectedYearId, grades, page, perPage) {
        return {
            ...
            async changeYear(yearId) {
                ...
                try {
                    const url = `{{ route('evaluator.pupils') }}?ajax=1&year_id=${encodeURIComponent(yearId)}&_t=${Date.now()}`;
            ...
            async previewImport() {
                ...
                try {
                    const response = await fetch(`{{ route('evaluator.pupils.import.preview') }}`, {
                        method: 'POST',
                        headers: {
            ...
            async commitImport() {
                ...
                try {
                    const response = await fetch(`{{ route('evaluator.pupils.import.commit') }}`, {
                        method: 'POST',
                        headers: {
            ...
            async addPupil() {
                ...
                try {
                    const response = await fetch(`{{ route('evaluator.pupils.store') }}`, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': '{{ csrf_token() }}',
            ...
            async updatePupil() {
                ...
                try {
                    const response = await fetch(`/evaluator/pupils/${this.editTarget.pupil_id}`, {
                        method: 'PATCH',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': '{{ csrf_token() }}',
            ...
            async dropPupil(pupil) {
                await this.patchStatus(`/evaluator/pupils/${pupil.pupil_id}/drop`, 'Pupil marked as dropped successfully.');
            },

            async restorePupil(pupil) {
                await this.patchStatus(`/evaluator/pupils/${pupil.pupil_id}/restore`, 'Pupil restored successfully.');
            },

            async archivePupil(pupil) {
                await this.patchStatus(`/evaluator/pupils/${pupil.pupil_id}/archive`, 'Pupil archived successfully.', true);
            },

            async deletePupil(pupil) {
                await this.patchStatus(`/evaluator/pupils/${pupil.pupil_id}`, 'Pupil deleted successfully.', true, 'DELETE');
            },
            ...
            async bulkDeletePupils() {
                ...
                try {
                    const response = await fetch(`/evaluator/pupils`, {
                        method: 'DELETE',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': '{{ csrf_token() }}',
