...
class PrincipalPupilsController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (! $section) {
            return response()->json([
                'message' => 'The selected section does not belong to your school, grade level, or school year.',
                'errors' => [
                    'section_id' => ['The selected section does not belong to your school, grade level, or school year.'],
                ],
            ], 422);
        }
    ...
    public function commitImport(Request $request)
    {
        ...
        $payloads = collect($validation['rows'])
            ->map(function ($row) use ($schoolId, $validated) {
                return [
                    'lrn' => $row['data']['lrn'],
    ...
    public function update(Request $request, string $pupilId)
    {
        ...
        if (! $existing) {
            return response()->json([
                'message' => 'Pupil not found in your school.',
            ], 404);
        }
        ...
        $response = Http::withHeaders(array_merge($this->supabaseHeaders(), [
            'Content-Type' => 'application/json',
            'Prefer' => 'return=representation',
        ]))->patch($this->supabaseUrl() . '/rest/v1/pupils?pupil_id=eq.' . $pupilId . '&school_id=eq.' . $schoolId, $payload);
    ...
    public function transferSection(Request $request, string $pupilId)
    {
        ...
        $response = Http::withHeaders(array_merge($this->supabaseHeaders(), [
            'Content-Type' => 'application/json',
            'Prefer' => 'return=representation',
        ]))->patch($this->supabaseUrl() . '/rest/v1/pupils?pupil_id=eq.' . $pupilId . '&school_id=eq.' . $schoolId, [
            'grade_level_id' => $validated['grade_level_id'],
            'section_id' => $validated['section_id'],
            'status' => 'enrolled',
        ]);
    ...
    public function delete(string $pupilId)
    {
        ...
        ]))->delete($this->supabaseUrl() . '/rest/v1/pupils?pupil_id=eq.' . $pupilId . '&school_id=eq.' . $schoolId);
    ...
    public function bulkDelete(Request $request)
    {
        ...
        if (count($existingIds) !== count($ids)) {
            return response()->json([
                'message' => 'One or more selected pupils were not found in your school.',
            ], 404);
        }

        $response = Http::withHeaders(array_merge($this->supabaseHeaders(), [
            'Prefer' => 'return=minimal',
        ]))->delete($this->supabaseUrl() . '/rest/v1/pupils?school_id=eq.' . $schoolId . '&pupil_id=in.(' . $this->postgrestInList($ids) . ')');
    ...
    {
        $grades = $this->fetchGradeLevels($schoolId);
    ...
    private function fetchSections(string $schoolId, string $yearId): array
    {
        ...
        if (! $response->successful()) {
            report('Failed to fetch class sections for principal pupils: ' . $response->body());
            return [];
        }
    ...
    private function fetchPupils(string $schoolId, array $sectionIds): array
    {
        ...
        if (! $response->successful()) {
            report('Failed to fetch pupils: ' . $response->body());
            return [];
        }
    ...
    private function findValidSection(string $schoolId, string $yearId, string $gradeLevelId, string $sectionId): ?array
    {
        ...
        if (! $response->successful()) {
            report('Failed to validate section: ' . $response->body());
            return null;
        }
    ...
    private function findSectionById(string $schoolId, string $sectionId): ?array
    {
        ...
        if (! $response->successful()) {
            report('Failed to fetch section by id: ' . $response->body());
            return null;
        }
    ...
    private function findPupil(string $schoolId, string $pupilId, bool $includeArchived = false): ?array
    {
        ...
        if (! $response->successful()) {
            report('Failed to fetch pupil: ' . $response->body());
            return null;
        }
