@props([
    'title' => 'District Consolidated Report',
    'report' => [],
])
...
<body>
    ...
    <main class="sheet">
        ...
        <section class="report-title">
            ...
            <div>DISTRICT CONSOLIDATED ORAL READING AND COMPREHENSION ASSESSMENT RESULT IN {{ $report['report_language_title'] ?? 'ENGLISH' }}</div>
        ...
        <section class="report-info">
            <div>
                <span class="label">District Supervisor:</span>
                <span class="value">{{ $report['supervisor_name'] ?? 'District Supervisor' }}</span>
            </div>
            <div>
                <span class="label">District:</span>
                <span class="value">{{ $report['district_name'] ?? 'District' }}</span>
            </div>
        </section>
        ...
        <table class="report-table">
            <thead>
                <tr>
                    <th rowspan="2" class="name">School</th>
                    <th colspan="2">Sex</th>
            ...
            <tbody>
                @forelse ($report['school_rows'] ?? [] as $row)
                    @php($summary = $row['summary'] ?? [])
                    <tr>
                        <td class="name">{{ $row['school_name'] ?? 'School' }}</td>
                ...
                @empty
                    <tr>
                        <td colspan="11">No submitted school reports found for this district summary.</td>
                    </tr>
                @endforelse
        ...
        <footer class="footer">
            ...
            <div class="footer-details">
                <div>Address: {{ $report['office_address'] ?? 'Not set' }}</div>
                <div>Contact: {{ $report['contact'] ?? 'Not set' }}</div>
                <div>Email: {{ $report['email'] ?? 'Not set' }}</div>
            </div>
        </footer>
    </main>
</body>
