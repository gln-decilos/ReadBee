...
class PrincipalProgressMonitoringController extends Controller
{
    public function index(Request $request)
    {
        ...
        $schoolYears = $this->fetchSchoolYears($schoolId);
        $selectedYearId = $request->query('year_id') ?: ($schoolYears[0]['year_id'] ?? null);
        $grades = $selectedYearId ? $this->buildGradeProgress($schoolId, $selectedYearId, $schoolYears) : [];
        ...
        return view('pages.principal.principal-progress-monitoring', [
    ...
    private function buildGradeProgress(string $schoolId, string $yearId, array $schoolYears): array
    {
        $sections = $this->fetchSections($schoolId, $yearId);

        if (empty($sections)) {
            return [];
        }

        $sectionIds = collect($sections)->pluck('section_id')->filter()->unique()->values()->all();
        ...
        $pupils = $this->fetchSchoolPupils($schoolId, $sectionIds);
        ...
        $sectionRows = collect($sections)
            ->map(function ($section) use ($gradeLevels, $pupilsBySection, $recordsByPupil, $schoolYear) {
                ...
                $pupilRows = $sectionPupils
                    ->map(function ($pupil) use ($recordsByPupil, $section, $grade) {
        ...
        return $sectionRows
            ...
            ->map(function ($gradeSections, $gradeLevelId) use ($gradeLevels, $schoolYear) {
    ...
    private function buildSummary(array $grades): array
    {
        $totalGrades = count($grades);
        $totalSections = collect($grades)->sum('sections_count');
        $totalPupils = collect($grades)->sum('total_pupils');
        $englishAssessed = collect($grades)->sum('english_assessed');
        $filipinoAssessed = collect($grades)->sum('filipino_assessed');
        $bothLanguages = collect($grades)->sum('both_languages_count');
        $totalRequired = collect($grades)->sum('total_required');
        $totalCompleted = collect($grades)->sum('total_completed');

        return [
            'grades_count' => $totalGrades,
    ...
    private function fetchSections(string $schoolId, string $yearId): array
    {
        ...
        if (! $response->successful()) {
            report('Failed to fetch principal progress sections: ' . $response->body());
            return [];
        }
    ...
    private function fetchSchoolPupils(string $schoolId, array $sectionIds): array
    {
        ...
        if (! $response->successful()) {
            report('Failed to fetch principal progress pupils: ' . $response->body());
            return [];
        }
    ...
    private function fetchAssessmentRecords(string $yearId, array $pupilIds): array
    {
        ...
        if (! $response->successful()) {
            report('Failed to fetch principal progress assessment records: ' . $response->body());
            return [];
        }
    ...
    private function fetchSchoolYears(string $schoolId): array
    {
        ...
        if (! $sectionsResponse->successful()) {
            report('Failed to fetch school year ids for principal progress monitoring: ' . $sectionsResponse->body());
            return [];
        }
        ...
        if (! $response->successful()) {
            report('Failed to fetch school year rows for principal progress monitoring: ' . $response->body());
            return [];
        }
    ...
    private function fetchRowsByIds(string $table, string $idField, array $ids, string $select): array
    {
        ...
        if (! $response->successful()) {
            report("Failed to fetch {$table} for principal progress monitoring: " . $response->body());
            return [];
        }
